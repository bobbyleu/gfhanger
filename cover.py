...
async def async_setup_entry(
    ...
    for device_info in client.devices_info:
        entities.append(GfCover(device_info, client))
...
class GfCover(CoverEntity):
    ...
    async def async_open_cover(self, **kwargs):
        await self._client.remote_control(self._device_info["_id"], 2)

    async def async_close_cover(self, **kwargs):
        await self._client.remote_control(self._device_info["_id"], 1)

    async def async_stop_cover(self, **kwargs):
        await self._client.remote_control(self._device_info["_id"], 3)
